...
module JSDOM.Generated.HTMLDataElement
       (setValue, getValue, HTMLDataElement(..), gTypeHTMLDataElement)
       where
...
setValue ::
         (MonadDOM m, ToJSString val) => HTMLDataElement -> val -> m ()
setValue self val = liftDOM (self ^. jss "value" (toJSVal val))

-- | <https://developer.mozilla.org/en-US/docs/Web/API/HTMLDataElement.value Mozilla HTMLDataElement.value documentation> 
getValue ::
         (MonadDOM m, FromJSString result) => HTMLDataElement -> m result
getValue self
